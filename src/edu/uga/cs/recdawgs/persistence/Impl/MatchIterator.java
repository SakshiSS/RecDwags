...
public class MatchIterator implements Iterator<Match> {
	...
	ObjectLayer ob=null;
	boolean more=false;
	public MatchIterator(ResultSet rs,ObjectLayer obj){
		this.rs=rs;
		this.ob=obj;
		try {
            more = rs.next();
	...
	public Match next() {
		...
		try {
			...
			System.out.println("In Iterator::"+away_points);
			m_date=rs.getDate(4);
		    ...
		    awayteamid=rs.getInt(32);
		    a_name=rs.getString(33);
		    ...
		    try {
			    ...
			    at.setParticipatesInLeague(l);
			     ...
			     m1.setId(id);
			     m1.setHomePoint(home_points);
			     m1.setAwayPoints(away_points);
			     m1.setDate(m_date);
			     m1.setIsCompleted(isCompleted);
			     m1.setHomeTeam(ht);
			     m1.setAwayTeam(at);
			     m1.setRound(rnd);
